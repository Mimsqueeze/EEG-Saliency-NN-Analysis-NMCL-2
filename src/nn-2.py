...
def main():
    ...
    for n in range(1, 30):
        ...
        combined_easy = easy if (combined_easy is None) else np.vstack((easy, combined_easy))

        diff = np.load(f"./features-coh-only/matrices/sub-{n:02}/diff.npy")
        combined_diff= diff if (combined_diff is None) else np.vstack((diff, combined_diff))
