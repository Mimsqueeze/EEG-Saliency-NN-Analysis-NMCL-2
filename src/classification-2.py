...
# Hyperparameters
SIGNIFICANCE = 1e-1 # Significance level for feature selection
MODEL = "SVM"
...
"""
The get_pvalues(easy, diff) function
...
"""
def get_pvalues(easy, diff):
    # we want to iterate through each feature, so transpose
    t_easy, t_diff = np.transpose(easy), np.transpose(diff)
    ...
    for i in range(len(t_easy)):
        # Get ith column
        easy_d, diff_d = t_easy.iloc[i], t_diff.iloc[i]

        # complete test for normality (Shapiro-Wilk test)
        _, easy_p = shapiro(easy_d)
        _, diff_p = shapiro(diff_d)

        # Normal distribution: Use independent t-test.
        # Non-normal distribution: Use Mann-Whitney U Test
        if (easy_p < 0.05 and diff_p < 0.05):
            _, p_value = mannwhitneyu(easy_d, diff_d)
        else:
            _, p_value = ttest_ind(easy_d, diff_d)   
...
def main():
    ...
    combined_diff["label"] = 1

    # Combine the easy and diff features into a single dataframe
    df = pd.concat([combined_easy, combined_diff])

    # Shuffle the rows of the dataframe
    df = df.sample(frac = 1)

    # Compute p values between columns
    pvals = get_pvalues(combined_easy, combined_diff)
    ...
    dir = f"./results/binary/"
    if not os.path.exists(dir): os.makedirs(dir)
    results.to_csv(f"{dir}/{MODEL}-{HYPERPARAMETERS}-{SIGNIFICANCE}-coh-only-2.csv", header=False)
