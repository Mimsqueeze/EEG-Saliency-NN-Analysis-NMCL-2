...
def main():
    ...
    for channel in range(NUM_CHANNELS): # for every frequency range
        ...
        avg_med_smap_dict[channel] = None
            ...
            if (test_acc >= 0.9):
                ...
                for channel in range(NUM_CHANNELS): # for every frequency range
                    for i in range(NUM_IMAGES): # for every image
                        ...
                        med_smap = get_saliency_map(model, tf.convert_to_tensor(med[i:i+1, :, :, :]), channel)
                        ...
                        avg_med_smap_dict[channel] = med_smap if (avg_med_smap_dict[channel] is None) else avg_med_smap_dict[channel] + med_smap
    ...
    base_dir = f"./plots/saliency maps/ternary/{MODEL_NAME}/"
    ...
    for channel in range(NUM_CHANNELS): # for every frequency range
        ...
        avg_med_smap_dict[channel] = avg_med_smap_dict[channel]/(NUM_IMAGES * K)
        avg_diff_smap_dict[channel] = avg_diff_smap_dict[channel]/(NUM_IMAGES * K)

        # Create directory if it doesn't exist already
        dir = f"./plots/saliency maps/ternary/{MODEL_NAME}/{RANGES[channel]}"
        if not os.path.exists(dir): os.makedirs(dir)
        
        # Save to file
        plot_m(matrix=avg_easy_image, title= f"Coherence for {RANGES[channel]} MATB-Easy", filename=f"{dir}/easy_image.svg")
        plot_m(matrix=avg_med_image, title= f"Coherence for {RANGES[channel]} MATB-Medium", filename=f"{dir}/med_image.svg")
        ...
        plot_m(matrix=avg_med_smap_dict[channel], title= f"Saliency Map for {RANGES[channel]} MATB-Medium", filename=f"{dir}/med_smap.svg")
        ...
        med_indices = np.where(np.triu(avg_med_smap_dict[channel] > threshold))
        ...
        med_list = list(zip(med_indices[0], med_indices[1]))
        diff_list = list(zip(diff_indices[0], diff_indices[1]))
        
        # Convert indices to list of string tuples
        easy_cells = list(map(lambda x: (f"{LABELS[x[0]]}-{LABELS[x[1]]}, S: {avg_easy_smap_dict[channel][x[0]][x[1]]}, E: {avg_easy_image[x[0]][x[1]]}, M: {avg_med_image[x[0]][x[1]]}, D: {avg_diff_image[x[0]][x[1]]}"), easy_list))
        med_cells = list(map(lambda x: (f"{LABELS[x[0]]}-{LABELS[x[1]]}, S: {avg_med_smap_dict[channel][x[0]][x[1]]}, E: {avg_easy_image[x[0]][x[1]]}, M: {avg_med_image[x[0]][x[1]]}, D: {avg_diff_image[x[0]][x[1]]}"), med_list))
        diff_cells = list(map(lambda x: (f"{LABELS[x[0]]}-{LABELS[x[1]]}, S: {avg_diff_smap_dict[channel][x[0]][x[1]]}, E: {avg_easy_image[x[0]][x[1]]}, M: {avg_med_image[x[0]][x[1]]}, D: {avg_diff_image[x[0]][x[1]]}"), diff_list))
        ...
        with open(f"{dir}/med_cells.txt", "w") as f:
            for item in med_cells:
                f.write(str(item) + '\n')
